      ...
      <div className="cma-section-container flex flex-col gap-12 items-center">
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full">
          {giftCards.map((card, i) => (
            <motion.div
              key={card.heading}
              className={`${card.bg} flex-1 rounded-[24px] px-[40px] md:px-[60px] py-[64px] flex flex-col items-center gap-8`}
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex flex-col gap-2 items-center">
                <span className={`${card.badge.badgeBg} ${card.badge.textColor} text-[10px] font-extrabold uppercase leading-none px-2 py-1 rounded-full tracking-wide`}>
                  {card.badge.label}
                </span>
                <h2 className={`${card.headingColor} text-center leading-none tracking-[-1px]`}>{card.heading}</h2>
              </div>
              <p className={`${card.bodyColor} text-center max-w-[400px]`}>{card.body}</p>
              <a href={card.cta.href} className={card.cta.style}>{card.cta.label}</a>
            </motion.div>
          ))}
        </div>
      </div>
