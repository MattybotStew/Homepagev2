...
import { type EventItem, eventFilters, eventsByCategory } from "../data/events";
...
export default function SpecialExperiences({
	...
	const events = (filterEvents[activeFilter] ?? filterEvents.All ?? []).slice(
		0,
		3,
	);
			...
			{wave && (
				...
				>
					{Array.from({ length: 10 }).map((_, i) => (
						<img
							key={`item-${i}`}
							src={wave === "white" ? imgWaveWhite : imgWaveNavy}
							alt=""
